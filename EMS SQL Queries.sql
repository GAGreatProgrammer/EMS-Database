...
QR.[QRCard_ID], IIF(QR.[Permanent_Card] = 1, N'Permanent', N'Temporary') AS [Permanent_Card], QR.[Change_Period], 
IIF(QR.[Blocked] = '0', N'Is not blocked', N'Is blocked') AS [Blocked]
...
A.Username, IIF(A.Permanent_Password = 1, N'Permanent', N'Temporary') AS [Permanent_Password], A.Password_Change_Period, A.Access_Level
...
IIF(ES.[Pay_Frequency] = 1, N'Monthly', IIF(ES.[Pay_Frequency] = 2, N'Every Week', N'Irregular Schedule')) AS [Pay_Frequency], 
...
SELECT EV.[ID], E.[Passport_ID], CONCAT(E.[Name], ' ', E.[Surname]) AS [FullName], 
       ...
       EV.[Date], EV.[Time], IIF(EV.[Type] = 1, N'Check In', N'Check Out') AS [Type]
FROM Employee E INNER JOIN Department D ON E.Department = D.ID
...
SELECT CONVERT(NVARCHAR(50) , E.[ID]) AS [EmployeeID], E.[Passport_ID], CONCAT(E.[Name], ' ', E.[Surname]) AS [FullName], 
       E.[Telephone], E.[Email], D.[Department], E.[Position], e.[Status],
       FORMAT(EV.[Date], 'dd/MM/yyyy') AS [Date], CAST(EV.[Time] AS NVARCHAR) AS [Time], IIF(EV.[Type] = 1, N'Check In', N'Check Out') AS [Type]
FROM Employee E INNER JOIN Department D ON E.Department = D.ID
                INNER JOIN EmployeeVisits EV ON E.ID = EV.EmployeeID
GO
